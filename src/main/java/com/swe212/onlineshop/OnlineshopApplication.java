    ...
    public void run(String... args) throws Exception {
        ...
        if (customerRepository.findAll().isEmpty() &&
            ...
            List<Product> products = List.of(
                    Product.builder().name("Laptop").supplier("Dell").price(new BigDecimal("1200.00")).imageUrl(null).build(),
                    Product.builder().name("Smartphone").supplier("Samsung").price(new BigDecimal("899.99")).imageUrl(null).build(),
                    Product.builder().name("Headphones").supplier("Sony").price(new BigDecimal("199.50")).imageUrl(null).build(),
                    Product.builder().name("Monitor").supplier("LG").price(new BigDecimal("350.75")).imageUrl(null).build(),
                    Product.builder().name("Keyboard").supplier("Logitech").price(new BigDecimal("75.00")).imageUrl(null).build()
            );
